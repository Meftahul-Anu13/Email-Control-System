...
def view_spam_emails(folder="[Gmail]/Spam", limit=10):
    try:
...
def move_from_spam():
    try:
        ...
        mail = imaplib.IMAP4_SSL(IMAP_SERVER, IMAP_PORT)
        mail.login(MICROSOFT_EMAIL, MICROSOFT_PASSWORD)
        mail.select("[Gmail]/Spam")  # Adjust folder for other providers

        # Search for all spam emails
        _, messages = mail.search(None, "ALL")
        email_ids = messages[0].split()

        # Logic to move emails to inbox
        for eid in email_ids:
            mail.store(eid, '+FLAGS', '\\Seen')  # Mark as read
            mail.copy(eid, "INBOX")  # Copy to inbox
            mail.store(eid, '+FLAGS', '\\Deleted')  # Delete from spam

        mail.expunge()
        mail.logout()
        return {"success": True, "message": "Moved emails from Spam to Inbox"}
    except Exception as e:
        return {"success": False, "message": str(e)}
